...
class NeuralNet:
    def __init__(self, dataFile, header=True, h=40, h2=20):
        ...
        self.W_hidden2 = 2 * np.random.random((h, h2)) - 1
    ...
    def train(self, max_iterations=6000, learning_rate=0.5):
        for iteration in range(max_iterations):
            ...
            update_weight_hidden2 = learning_rate * np.dot(self.X_hidden.T, self.deltaHidden2)
            update_weight_hidden2_b = learning_rate * np.dot(np.ones((np.size(self.X, 0), 1)).T, self.deltaHidden2)
            ...
            self.W_hidden2 += update_weight_hidden2
    ...
    def forward_pass(self,xValue = 0, activation="sigmoid"):
        ...
        in_hidden2 = np.dot(self.X_hidden, self.W_hidden2) + self.Wb_hidden2
        # TODO: I have coded the sigmoid activation, you have to do the rest
        if activation == "sigmoid":
    ...
    def backward_pass(self, out, activation):
        ...
        self.compute_hidden_delta2(activation)
    ...
    def compute_hidden_delta(self, activation="sigmoid"):
        if activation == "sigmoid":
            delta_hidden_layer = (self.deltaHidden2.dot(self.W_hidden2.T)) * (self.__sigmoid_derivative(self.X_hidden))
        elif activation == "tanh":
            delta_hidden_layer = (self.deltaHidden2.dot(self.W_hidden2.T))*(self.__tanh_derivative(self.X_hidden))
        elif activation == "relu":
            delta_hidden_layer = (self.deltaHidden2.dot(self.W_hidden2.T))*(self.__relu_derivative(self.X_hidden))
        self.deltaHidden = delta_hidden_layer

    def compute_hidden_delta2(self, activation="sigmoid"):
        if activation == "sigmoid":
